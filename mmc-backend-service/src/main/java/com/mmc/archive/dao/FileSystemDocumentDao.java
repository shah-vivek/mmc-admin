...
public class FileSystemDocumentDao implements IDocumentDao {
     ...
     * 
     * @see com.mmc.archive.dao.IDocumentDao#insert(com.mmc.archive.service.Document)
     */
    @Override
     ...
     * the parameter.
     * 
     * @see com.mmc.archive.dao.IDocumentDao#findByPersonNameDate(java.lang.String, java.util.Date)
     */
    @Override
     ...
     * 
     * @see com.mmc.archive.dao.IDocumentDao#load(java.lang.String)
     */
    @Override
    ...
    @Override
    public List<Document> albumsDoc(String albumId) {
        try {
            Iterable<DocumentMetadata> docs = findUsingAlbumId(albumId);
            List<Document> docList = new ArrayList<Document>();
            for(DocumentMetadata doc : docs){
                docList.add(loadFromFileSystem(doc.getUuid()));
            }
            return docList;
        }catch (IOException e){
            String message = "Error while loading album with id: " + albumId;
            LOG.error(message, e);
            throw new RuntimeException(message, e);
        }
    }

    private List<DocumentMetadata> findUsingAlbumId(String albumId) throws IOException{
        ...
        for (String uuid : uuidList) {
            ...
            if(isAlbumMatched(metadata, albumId)) {
    ...
    private boolean isAlbumMatched(DocumentMetadata metadata, String albumId) {
        if(metadata==null) {
            return false;
        }
        boolean match = true;
        if(albumId!=null) {
            match = (albumId.equals(metadata.getAlbumId()));
        }
        return match;
    }
